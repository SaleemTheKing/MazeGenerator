...
public class MazeGenerator : MonoBehaviour
{
    ...
    void CreateGrid()
    {
        ...
        _wallFolder.AddComponent<MazeRotate>();

        _initPos = new Vector3((-xSize / 2) + (wallLength / 2), 0, (-ySize / 2) + (wallLength / 2));

        var credits = Instantiate(creditsPrefab, new Vector3(0,2,0), Quaternion.identity);
        // credits.transform.localScale = new Vector3(0.5f, 0.5f, 0.5f);
        credits.transform.SetParent(_wallFolder.transform);
    ...
    public void RegenerateMaze()
    {
        ...
        Start();
        orbitalCamera.GetComponent<CameraOverview>().SetDistance();
    }
}